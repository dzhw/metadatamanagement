...
const documents = db.fs.files.aggregate([
  {
    $match: {
      "metadata.type.en": "Variable Questionnaire"
    }
  },
  {
    $lookup: {
...
documents.forEach(document => {
  ...
  const updatedDescriptionDe = `Variablenfragebogen zu “${dataPackageTitleDe}”, ${pluralSuffixDe}: ${surveyNumbers}`;
  const updatedDescriptionEn = `Variable Questionaire of “${dataPackageTitleEn}”, ${pluralSuffixEn}: ${surveyNumbers}`;
