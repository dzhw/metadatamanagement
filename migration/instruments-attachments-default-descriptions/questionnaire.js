...
const documents = db.fs.files.aggregate([
  {
    $match: {
      "metadata.type.en": "Questionnaire"
    }
  },
  {
    $lookup: {
...
documents.forEach(document => {
  ...
  const updatedDescriptionDe = `Erhebungsinstrument zu “${dataPackageTitleDe}”, ${pluralSuffixDe}: ${surveyNumbers}`;
  const updatedDescriptionEn = `Instrument of “${dataPackageTitleEn}”, ${pluralSuffixEn}: ${surveyNumbers}`;
