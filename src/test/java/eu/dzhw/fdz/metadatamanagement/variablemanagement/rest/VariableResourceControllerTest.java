...
public class VariableResourceControllerTest extends AbstractTest {
  ...
  public void testCreateDateVariableWithContinousScaleLevel() throws Exception {
    ...
    Variable variable =
        UnitTestCreateDomainObjectUtils.buildVariable(project.getId(), 1, "var1", 1, surveyNumbers);
    variable.setDataType(DataTypes.DATE);
    variable.setScaleLevel(ScaleLevels.RATIO);
    List<ValidResponse> validResponses = new ArrayList<>();
    variable.getDistribution().setValidResponses(validResponses);
    variable.getDistribution().getStatistics().setMaximum("2017-01-31");
    variable.getDistribution().getStatistics().setMinimum("2017-01-30");
    ...
    mockMvc
        ...
        .andExpect(jsonPath("$.errors[0].message", containsString(
            "variable-management.error.variable.restricted-scale-level-for-date-data-type")));
  }
  ...
  public void testCreateVariableWithoutLabel() throws Exception {
    ...
    mockMvc
        ...
        .andExpect(status().isBadRequest()).andExpect(jsonPath("$.errors[0].message",
            containsString("variable-management.error.variable.label.i18n-string-not-empty")));
  }
  ...
  public void testGenerationDetailsWithExpressionLanguageButWithoutRule() throws Exception {
    ...
    mockMvc
        ...
        .andExpect(status().isBadRequest())
        .andExpect(jsonPath("$.errors[0].message", containsString(
            "variable-management.error.generation-details.rule-expression-language-and-rule-filled-or-empty")));
  }
  ...
  public void testCreateVariableWithNonUniqueCode() throws Exception {
    ...
    Variable variable =
        UnitTestCreateDomainObjectUtils.buildVariable(project.getId(), 1, "var1", 1, surveyNumbers);
    Missing value1 = variable.getDistribution().getMissings().get(0);

    Missing value2 = Missing.builder().code(value1.getCode()).label(value1.getLabel())
        .absoluteFrequency(value1.getAbsoluteFrequency())
        .relativeFrequency(value1.getRelativeFrequency()).build();
  ...
  public void testCreateVariableWithNonUniqueValueClass() throws Exception {
    ...
    ValidResponse value2 = ValidResponse.builder().value(validResponse.getValue())
        .label(validResponse.getLabel()).absoluteFrequency(validResponse.getAbsoluteFrequency())
        .relativeFrequency(validResponse.getRelativeFrequency())
        .validRelativeFrequency(validResponse.getValidRelativeFrequency()).build();

    variable.getDistribution().getValidResponses().add(value2);
  ...
  public void testCreateVariableWithInvalidDataType() throws Exception {
    ...
    Variable variable =
        UnitTestCreateDomainObjectUtils.buildVariable(project.getId(), 1, "var1", 1, surveyNumbers);
    variable.setDataType(I18nString.builder().de("InvalidValue").en("InvalidValue").build());
  ...
  public void testUpdateVariable() throws Exception {
    ...
    List<Integer> surveyNumbers = new ArrayList<>();
    surveyNumbers.add(1);
    Variable variable = UnitTestCreateDomainObjectUtils.buildVariable(project.getId(), 1,
        "bdem08_v1", 1, surveyNumbers);
    ...
    mockMvc.perform(get(API_VARIABLES_URI + "/" + variable.getId())).andExpect(status().isOk())
        .andExpect(jsonPath("$.id", is(variable.getId()))).andExpect(jsonPath("$.version", is(1)))
  ...
  public void testFilterExpressionLanguageWithSpelLanguage() throws Exception {
    ...
    Variable variable =
        UnitTestCreateDomainObjectUtils.buildVariable(project.getId(), 1, "var1", 1, surveyNumbers);
    variable.getFilterDetails().setExpressionLanguage(FilterExpressionLanguages.SPEL);
  ...
  public void testRuleExpressionLanguageWithWrongLanguage() throws Exception {
    ...
    Variable variable =
        UnitTestCreateDomainObjectUtils.buildVariable(project.getId(), 1, "var1", 1, surveyNumbers);
    variable.getGenerationDetails().setRuleExpressionLanguage("WrongLanguage");
  ...
  public void testRuleExpressionLanguageWithStataLanguage() throws Exception {
    ...
    Variable variable =
        UnitTestCreateDomainObjectUtils.buildVariable(project.getId(), 1, "var1", 1, surveyNumbers);
    variable.getGenerationDetails().setRuleExpressionLanguage(RuleExpressionLanguages.STATA);
  ...
  public void testVariableWithDataTypeDate() throws Exception {
    ...
    Variable variable =
        UnitTestCreateDomainObjectUtils.buildVariable(project.getId(), 1, "var1", 1, surveyNumbers);
    variable.setDataType(DataTypes.DATE);
    variable.getDistribution().setValidResponses(null);
    List<ValidResponse> validResponses = new ArrayList<>();
    validResponses.add(ValidResponse.builder()
        .label(I18nString.builder().de("Deutsches Label").en("English Label").build())
        .absoluteFrequency(1234).relativeFrequency(87.5).validRelativeFrequency(88.9)
        .value(LocalDateTime.now().toString()).build());
  ...
  public void testCreateVariableShadowCopy() throws Exception {
    ...
    mockMvc
        .perform(post(API_VARIABLES_URI).content(TestUtil.convertObjectToJsonBytes(variable))
            .contentType(MediaType.APPLICATION_JSON))
        .andExpect(status().isBadRequest()).andExpect(jsonPath("$.errors[0].message",
