...
describe('Controllers Tests ', function() {
  ...
  describe('HealthController', function() {
      ...
      var ElasticSearchAdminService; // local utility functions
      ...
      beforeEach(inject(function($injector) {
          ...
          spyOn(ElasticSearchAdminService, 'recreateAllElasticsearchIndices')
          .and.callThrough();
          spyOn($uibModal, 'open').and.callThrough();
          createController();
        }));
      describe('isHealthObject and hasSubSystem', function() {
          it('should verify empty object is not a health property leaf',
          function() {
              expect($scope.isHealthObject({})).toBe(false);
              expect($scope.hasSubSystem({})).toBe(false);
            });
          it('should verify object with property status and no subsystems' +
          ' is a health property leaf', function() {
              var healthObject = {
                  'status': 'UP'
                };
          ...
          it('should verify that object property status and unrecognized' +
          ' objects is a health property leaf', function() {
              var healthObject = {
                  'status': 'UP',
                  'subsystem': {
                      'hello': 'UP'
          ...
          it('should verify object with property status but with subsystems' +
          ' is NOT a health property leaf', function() {
              var healthObject = {
                  'status': 'UP',
                  'subsystem': {
                      'status': 'UP'
      ...
      describe('transformHealthData', function() {
          ...
          it('should flatten health data with subsystems at level 1, main' +
          ' system has no additional information', function() {
              var data = {
                  'status': 'UP',
                  'db': {
                      'status': 'UP',
          ...
          it('should flatten health data with subsystems at level 1, main' +
          ' system has additional information', function() {
              var data = {
                  'status': 'UP',
                  'db': {
                      'status': 'UP',
          ...
          it('should flatten health data with subsystems at level 1, main' +
          ' system has additional error', function() {
              var data = {
                  'status': 'UP',
                  'db': {
                      'status': 'UP',
      ...
      describe('getModuleName(path, name)', function() {
          it('should show both path and name if defined', function() {
              expect($scope.getModuleName('path', 'name')).toEqual('path' +
          ...
          it('should show empty string if neither name nor path is defined',
          function() {
            expect($scope.getModuleName()).toEqual('');
          });
        });
      describe('MonitoringService, ElasticSearchAdminService and scope',
      function() {
