...
xdescribe('Directive Tests ', function() {
  ...
  beforeEach(inject(function($compile, $rootScope, $injector) {
    spyOn(localStorage, 'getItem').and.callFake(function(key) {
      return
        "{\"access_token\":\"79b4ddc8-eb1c-4e7f-82e0-0dc2038a56fd\"}";
    });
  ...
  describe('Password strength', function() {
    ...
    it("Should change the first 2 points of the strength bar", function() {
      ...
      var firstpointStyle = elm.find('ul').children('li')[0].getAttribute(
        'style');
      expect(firstpointStyle).toContain(
        'background-color: rgb(255, 153, 0)');

      var secondpointStyle = elm.find('ul').children('li')[1].getAttribute(
        'style');
      expect(secondpointStyle).toContain(
        'background-color: rgb(255, 153, 0)');

      var thirdpointStyle = elm.find('ul').children('li')[2].getAttribute(
        'style');
      expect(thirdpointStyle).toContain(
    ...
    it("Should change the first 4 points of the strength bar", function() {
      ...
      var fourthpointStyle = elm.find('ul').children('li')[3].getAttribute(
        'style');
      expect(fourthpointStyle).toContain(
        'background-color: rgb(153, 255, 0)');

      var fifthpointStyle = elm.find('ul').children('li')[4].getAttribute(
        'style');
      expect(fifthpointStyle).toContain(
        'background-color: rgb(221, 221, 221)');
    });
  });
});
