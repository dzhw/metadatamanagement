...
angular
  ...
  .factory(
    'AuthServiceProvider',
    function loginService($http, localStorageService, $window, $q, AuthProperties) {
      var config = {
        ...
        scope: 'openid email profile role_admin role_data_provider role_publisher role_release_manager'
      };
      ...
      return {
        ...
        isLoggedInSso: function () {
          ...
          $http.get(
            AuthProperties.issuer + '/user/login?_format=json',
            {
              headers: {
        ...
        refreshToken: function () {
          ...
          if (localStorageService.get('tokens')) {
            var data =
              ...
              '&refresh_token=' + localStorageService.get('tokens').refresh_token +
              '&client_id=' + encodeURIComponent(config.clientId) +
