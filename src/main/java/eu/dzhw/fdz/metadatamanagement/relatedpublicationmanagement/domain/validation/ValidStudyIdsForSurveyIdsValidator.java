...
/**
 * Ensure that a related publication is linked to the study of a survey.
 * 
 * @author René Reitmann
 */
@RequiredArgsConstructor
public class ValidStudyIdsForSurveyIdsValidator
    implements ConstraintValidator<ValidStudyIdsForSurveyIds, RelatedPublication> {
   ...
   */
  @Override
  public void initialize(ValidStudyIdsForSurveyIds constraintAnnotation) {
  ...
  public boolean isValid(RelatedPublication relatedPublication,
    ...
    List<String> invalidIds = relatedPublication.getSurveyIds().stream()
        .filter(surveyId -> {
          Survey survey = surveyRepository.findById(surveyId).orElse(null);
          if (survey == null || relatedPublication.getStudyIds().contains(survey.getStudyId())) {
            return false;
          }
          return true;
