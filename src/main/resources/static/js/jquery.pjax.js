...
(function($) {
 ...
 // Ignore event with default prevented
	...
	function pjax(options) {
		...
		options.success = function(data, status, xhr) {
			...
			// Only blur the focus if the focused element is within the
			// container.
 ...
 // Since state IDs always increase, we can deduce the navigation direction
	...
	function cloneContents(container) {
		...
		// Unmark script tags as already being eval'd so they can get executed
		// again
	...
	function extractContainer(data, xhr, options) {
		...
		obj.url = serverUrl ? stripInternalParams(parseURL(serverUrl))
				: options.requestUrl
	...
	$.support.pjax = window.history
			...
			!navigator.userAgent
					.match(/((iPod|iPhone|iPad).+\bOS\s+[1-4]\D|WebApps\/.+CFNetwork)/)
